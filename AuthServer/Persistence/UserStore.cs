...
namespace AuthServer.Persistence
{
    ...
    public class UserStore : IUserStore
    {
        ...
        public UserStore(IConfiguration configuration, IdentityContext context)
        {
            _context = context;
        ...
        {
            var login = await _context.Users.Where(t => t.UserName == username)
                .Include(r => r.UserPassword)
                .FirstOrDefaultAsync();

            if (login == null)
                return false;

            var hashPassword = AppUser.Create(password, username);

            if (!login.UserPassword.Any() || login.UserPassword.Last().Password != hashPassword)
                return false;
        ...
        {
            AppUser user = null;
            using (SqlCommand cmd = new SqlCommand("SELECT * FROM [users] WHERE [ProviderName] = @pname AND [ProviderSubjectId] = @psub;"))
            {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                if (reader.HasRows)
                {
                    ...
                    user = new AppUser()
                    {
                        ...
                        SubjectId = Convert.ToString(userRow["userId"]),
                    ...
                    {new Claim(JwtClaimTypes.Subject, user.id.ToString()),
                        new Claim(JwtClaimTypes.Name, user.Username),
                        new Claim(JwtClaimTypes.GivenName, "Jignesh"),
                        new Claim(JwtClaimTypes.FamilyName, "chovatia"),
